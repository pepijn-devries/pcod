...
source('config.R')
source('MakeInitScenarioHP_NL_Spring_622.r')
...
# NdistSum is the total population ss1 = unlist(strsplit(s, split='_', fixed=TRUE))ize for the disturbed population each year, NNotDistSum is the equivalent for the matching undisturbed population
    ...
    for(t in 1:pile_years){ 
      ...
      for(i in 1:nvulnmean){ # start the loop over the multiple vulnerable sub-pops; 
        if(Day1){
          ...
          yrs <- which(yearvec == t)
          tmp <- f.disturbance_and_PTS.Model2(pile[yrs, max(pilesx) + 2  + i], vuln[i], days, p_disturb[yrs, i], p_pts[yrs, i], prop_dist) 
        } else {
          # otherwise call f.disturbance_and_PTS
          yrs <- which(yearvec == t)
          tmp       <- f.disturbance_and_PTS(pile[yrs, max(pilesx) + 2  + i], vuln[i], days, p_disturb[yrs, i], p_pts[yrs, i], prop_dist)
        }
